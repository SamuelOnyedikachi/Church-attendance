  ...
  return (
    <section className="space-y-6">
      <header className="page-header">
        <div>
          <h1 className="page-title">Attendance for {service?.title || 'Service'}</h1>
          <p className="page-description">Review detailed check-in records and export them for reporting.</p>
        </div>
      ...
      <div className="table-card">
              ...
              {attendance.map((a) => (
                <tr key={a._id} className="table-row">
                  ...
                  <td className="px-4 py-3">
                    <span className={`pill ${a.firstTimer === 'Yes' ? 'pill-warning' : 'pill-neutral'}`}>
                      {a.firstTimer || '-'}
                    </span>
                  </td>
        ...
        {attendance.length === 0 && <p className="p-4 text-gray-500">No attendance recorded yet.</p>}
      </div>
    </section>
  );
}
